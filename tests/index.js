import { get_seeded_random_generator, get_starting_cells } from "../index.js";
...
function get_board_state() {
  return [0, 1, 2, 3].map((i) =>
    Array.from(document.querySelectorAll(`[data-row="${i}"]`)).map(
      (x) => String.prototype.trim(x.textContent) | 0
    )
  );
}
...
QUnit.module("UI updates", function (hooks) {
  ...
  QUnit.test(
    "User clicks on the new game button and the game starts, with two cells on the board, and a zero score",
    function (assert) {
        const { new_game_button } = get_ui_elements();
        ...
        const current_score = get_current_score();
debugger
        const {is_board_state_valid} = board_state.reduce((acc, row) => { 
            const {is_board_state_valid, count} = acc;
            const row_count = row.reduce((acc, number) => ['2', '4'].includes(number) ? acc + 1 : acc, 0);
            return {is_board_state_valid: is_board_state_valid && count + row_count === 2, count: count + row_count};
        }, {is_board_state_valid: false, count: 0});
debugger
        assert.deepEqual(current_score, 0, "Current score is 0");
        assert.deepEqual(is_board_state_valid, true, "There are two cells on the board with the expected values");
    }
  );
});
...
QUnit.module("Random number generation", function (hooks) {
  ...
  QUnit.test(
    "When starting the game, the first two cells are generated with the right probabilities",
    function (assert) {
