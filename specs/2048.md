...
Properties:
- cells on the board are either empty or powers of 2
  - AG (FORALL r, c: (is_empty(r, c) OR (cell(r, c) > 0 AND is_power_of_2(cell(r, c)))))
- If the game has not started, there are no non-empty cells on the board
  - AG (is_not_started IMPLIES (FORALL r, c: is_empty(r, c)))
- If the game is being played, there are at least two non-empty cells on the board
  - AG (is_playing IMPLIES (num_non_empty >= 2))
- If the game is being played, and the user made a move in a given direction, there are no empty cells in between non-empty cells in the given direction
  - AG (is_playing IMPLIES FORALL dir: AX (board_is_compacted(dir)))
...
- If the game is being played, and the user made a move in a given direction, and the score increased, then the board moved and the number of non-empty cells is equal or lower than before the move
...
- If the game is being played, and the user made a move in a given direction, and the board after the move is the same as before the move, then the scores remains the same
  - AG (is_playing IMPLIES FORALL dir: AX (NOT board_changed_from_prev IMPLIES score_unchanged_from_prev))
- If the game is being played, and the user made a move in a given direction, and the board changed, then the difference in the sum of the cells is 2 or 4 (with 90/10% probability)
  - AG (is_playing IMPLIES FORALL dir: AX (board_changed_from_prev IMPLIES (SumOfCells(current) - SumOfCells(prev) = 2 OR SumOfCells(current) - SumOfCells(prev) = 4)))
  - can't express probaility with formula so left to AI
- the current score never decreases
...
- best score is always superior or equal to current score
  - AG (best_score >= score)
- if the game is being played, and the user made a move in a given direction, and the board changed, and no further move will change the board, and there is no cell that is 2048, then the game is over (lost)
...
- if the game is being played, and the user made a move in a given direction, and the board changed, and there is a cell that is 2048, then the game is over (won)
  - AG ( is_playing IMPLIES FORALL dir: AX ( (board_changed_from_prev AND has_2048_cell) IMPLIES (game_over AND game_won) ) )
- if a game is over, and the user made a move in a given direction, scores and board do not change
  - AG (game_over IMPLIES AX (board_unchanged_from_prev AND score_unchanged_from_prev AND best_score_unchanged_from_prev))
- if a game is over, and the user starts a new game, the score is reset to 0, and the board is reset to two cells, and the best score does not change.
